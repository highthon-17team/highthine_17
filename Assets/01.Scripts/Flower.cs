...
public class Flower : MonoBehaviour
{
    ...
    private float spawnInterval = 5f; // 꽃 생성 간격
    ...
    void Update()
    {
        ...
        {
            GameObject randomFlower_1 = flowerArray[Random.Range(0, flowerArray.Length)]; 
            GameObject randomObject_1 = array_1[Random.Range(0, array_1.Length)]; 
            Instantiate(randomFlower_1, randomObject_1.transform.position, Quaternion.identity);
            lastSpawnTime = Time.time;
        }
        if(Sw_2.intensity >= 0.7f && Time.time - lastSpawnTime >= spawnInterval)
        {
            GameObject randomFlower_2 = flowerArray[Random.Range(0, flowerArray.Length)]; 
            GameObject randomObject_2 = array_2[Random.Range(0, array_2.Length)]; 
            Instantiate(randomFlower_2, randomObject_2.transform.position, Quaternion.identity);
            lastSpawnTime = Time.time;
        }
        if(Sw_3.intensity >= 0.7f && Time.time - lastSpawnTime >= spawnInterval)
        {
            GameObject randomFlower_3 = flowerArray[Random.Range(0, flowerArray.Length)]; 
            GameObject randomObject_3 = array_3[Random.Range(0, array_3.Length)]; 
            Instantiate(randomFlower_3, randomObject_3.transform.position, Quaternion.identity);
            lastSpawnTime = Time.time;
        }
    }
}
